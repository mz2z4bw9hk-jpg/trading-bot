...
| `models.ensemble.CalibratedEnsemble` | `fit(X, y, dates, w, t1)`; weights + calibrator from purged K-fold OOF, members refit on full train; `predict_proba` calibrated; `uncertainty` = member disagreement; `probability_interval` = Venn-ABERS [p0, p1] against the pooled OOF sample |
| `regime.RegimeDetector` | `fit(train bench)` then frozen, causal `transform`; never a model feature |
| `signals.SignalGenerator` | returns `None` unless every gate passes (incl. the LOWER Venn-ABERS bound when `conservative_gate`); a `Signal` is complete or absent |
...
| `monitor.paper.PaperTrackingStore` | append-only, idempotent per (symbol, date); resolution reuses `triple_barrier_labels` verbatim so tracked outcomes cannot drift from training labels |
